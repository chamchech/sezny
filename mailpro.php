...
$headers = array(
    "Authorization: Bearer $api_key_pro",
...
$resp=CurlSendGetRequest($api_url_pro.$data['fileid']."/download",$headers);
    // $resp=CurlSendGetRequest($api_url.$data["data"]['member']."/download",$headers);
    $resp=base64_decode($resp);
    // Email envoyé au client
    
    $message ="Merci d'avoir choisi Sezny Pro<br>" ;
    ...
    try {
      //SMTP CONFIG
        ...
        $mail->Subject     = 'Souscription Sezny Pro'; //Your mail subject
        $mail->Body        = $message;
        $mail->AltBody     = 'This is the body in plain text for non-HTML mail clients';
        $mail->WordWrap    = 50; // set word wrap to 50 characters
        $mail->AddAddress($data['mailid']);
        $mail->AddAddress($owner_email_pro);
        $mail->send();
    } catch (Exception $e) {

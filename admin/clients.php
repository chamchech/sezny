...
if(isset($_GET['delete'])){
    ...
    $stmt = $sql->prepare("delete from ventes where id=?");
...
if(isset($_GET["download"])){
    ...
    $stmt = $sql->prepare("SELECT files from ventes where id=?");
    $stmt->bindParam(1, $sellerid, PDO::PARAM_INT);
    $stmt->execute();
    $data = $stmt->fetchAll();
                    ...
                    <div class="col-md-12">
                        ...
                        <table class="table table-striped" id="tb1">
                            ...
                            <tbody>
                                <?php
                                    if(isset($_GET['sid'])){
                                        $stmt = $sql->prepare("select * from ventes where vendeur_id = ? order by id asc");
                                        $stmt->bindParam(1, $_GET['sid'], PDO::PARAM_INT);
                                    }else if(isset($_POST['but_search']) && $_POST['fromdate'] != '' && $_POST['enddate'] != ''){
                                        $sql_str = "select * from ventes where";
                                        ...
                                        $stmt = $sql->prepare("select * from ventes order by id asc");
                                    }
